...
export default function EnrollmentsRoutes(app) {
  ...
  app.post("/api/enrollments/current", async (req, res) => {
    ...
    if (!currentUser || !currentUser._id) {
      console.warn("POST /api/enrollments/current - No current user in session");
      return res.status(400).send({
    ...
    if (!courseId) {
      return res.status(400).send({ error: "Missing courseId in request body" });
    }

    try {
      const updatedEnrollments = await enrollmentsDao.toggleUserInCourse(currentUser._id, courseId);
